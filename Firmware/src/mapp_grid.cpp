...
bool mapping_grid_midi_recive(void* mapping_ptr, midi_msg_t* midi_msg_ptr) {
  mapp_grid_t* grid_ptr = (mapp_grid_t*)mapping_ptr;
  if (midi_msg_ptr->channel == grid_ptr->params.recive_chan) {
    return true;
  }
  return false;
};
...
// IN PROGRESS!
// Populates the MIDI grid layout with the incomming MIDI notes/chord coming from a regular MIDI keyboard plugged in the e256 HARDWARE_MIDI_INPUT
...
void mapping_grid_midi_dispose(void* mapping_ptr, midi_msg_t* midi_msg_ptr) {
  mapp_grid_t* grid_ptr = (mapp_grid_t*)mapping_ptr;
  grid_ptr->active_midi_msg_count--;
  if (grid_ptr->active_midi_msg_count == 0) {  // Save/rescure all llist nodes
...
void mapping_grid_create(const JsonObject &config) {
  ...
  grid_ptr->common.midi_recive_func_ptr = &mapping_grid_midi_recive; // TESTING!
  ...
  grid_ptr->params.recive_chan = config["recive"].as<uint8_t>();
