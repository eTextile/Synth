...
void median(void) {
  ...
  for (blob_t* blob_ptr = (blob_t*)ITERATOR_START_FROM_HEAD(&blobs); blob_ptr != NULL; blob_ptr = (blob_t*)ITERATOR_NEXT(blob_ptr)) {
    ...
    float inputVal = blob_ptr->centroid.Z; // The new value
    float outputVal = inputVal;            // The new value could be the median

    // Initialize all arrays
    if (blob_ptr->state && !blob_ptr->lastState) {
      memset(blobMedian[blob_ptr->UID].zVal, blob_ptr->centroid.Z, MEDIAN_WINDOW * sizeof(uint8_t));
      memset(blobMedian[blob_ptr->UID].zSort, blob_ptr->centroid.Z, MEDIAN_WINDOW * sizeof(uint8_t));
      ...
      blobMedian[blob_ptr->UID].zVal[blobMedian[blob_ptr->UID].index] = blob_ptr->centroid.Z;   // Store new value
      ...
      do {
        if (blob_ptr->centroid.Z <= blobMedian[blob_ptr->UID].zVal[index] &&
            lastOrdZ > blobMedian[blob_ptr->UID].zSort[index]) {
          blobMedian[blob_ptr->UID].zSort[index]++;     // Remove bigger value, add smaller value
        }
        else if (blob_ptr->centroid.Z > blobMedian[blob_ptr->UID].zVal[index] &&
                 lastOrdZ < blobMedian[blob_ptr->UID].zSort[index]) {
    ...
    blob_ptr->centroid.Z = outputVal;                                         // Replace the value with the computed blobMedian value
