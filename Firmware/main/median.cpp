...
void median(llist_t* blobs_ptr) {
  ...
  for (blob_t* blob_ptr = (blob_t*)ITERATOR_START_FROM_HEAD(blobs_ptr); blob_ptr != NULL; blob_ptr = (blob_t*)ITERATOR_NEXT(blob_ptr)) {
    ...
    if (blob_ptr->state && !blob_ptr->lastState) {
      ...
      memset(blobMedian[blob_ptr->UID].zOrd, blob_ptr->box.D, MEDIAN_WINDOW * sizeof(uint8_t));
      ...
      uint8_t lastOrdZ = blobMedian[blob_ptr->UID].zOrd[blobMedian[blob_ptr->UID].index]; // Save last order

      blobMedian[blob_ptr->UID].zVal[blobMedian[blob_ptr->UID].index] = blob_ptr->box.D;  // Store new value
      blobMedian[blob_ptr->UID].zOrd[blobMedian[blob_ptr->UID].index] = 0;                // Reset order number for new value
      ...
      do {
        if (blob_ptr->box.D <= blobMedian[blob_ptr->UID].zVal[index] &&
            lastOrdZ > blobMedian[blob_ptr->UID].zOrd[index]) {
          blobMedian[blob_ptr->UID].zOrd[index]++;     // Remove bigger value, add smaller value
        }
        else if (blob_ptr->box.D > blobMedian[blob_ptr->UID].zVal[index] &&
                 lastOrdZ < blobMedian[blob_ptr->UID].zOrd[index]) {
          blobMedian[blob_ptr->UID].zOrd[index]--;                            // Remove smaller value, add bigger value
        }
        if (blobMedian[blob_ptr->UID].zOrd[index] == MEDIAN_POS) {
          outputVal = blobMedian[blob_ptr->UID].zVal[index];                  // blobMedian found
        }
        if (blob_ptr->box.D > blobMedian[blob_ptr->UID].zVal[index]) {
          blobMedian[blob_ptr->UID].zOrd[blobMedian[blob_ptr->UID].index]++;  // Compute new value order
        }
