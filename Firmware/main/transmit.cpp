...
void blobs_usb_slipOsc(llist_t* blobs_ptr, preset_t* preset_ptr) {
  ...
  for (blob_t* blob_ptr = ITERATOR_START_FROM_HEAD(blobs_ptr); blob_ptr != NULL; blob_ptr = ITERATOR_NEXT(blob_ptr)) {
    ...
    msg.add(blob_ptr->box.D - preset_ptr->val);
    OSCbundle.add(msg);
  }
    ...
    if (blob_ptr->UID == blobs_ptr->index) {
      ...
      switch (preset_ptr->val) {
        case 1:
          ...
          break;
        case 2:
          usbMIDI.sendControlChange(2, (uint8_t)round(blob_ptr->centroid.X), blobs_ptr->index + 1);
          break;
        case 3:
          usbMIDI.sendControlChange(3, (uint8_t)round(blob_ptr->centroid.Y), blobs_ptr->index + 1);
          break;
        case 4:
...
void blobs_usb_midi_play(llist_t* blobs_ptr) {
  ...
  for (blob_t* blob_ptr = ITERATOR_START_FROM_HEAD(blobs_ptr); blob_ptr != NULL; blob_ptr = ITERATOR_NEXT(blob_ptr)) {
    ...
    usbMIDI.sendControlChange(2, (uint8_t)round(blob_ptr->centroid.X), blob_ptr->UID + 1);
    usbMIDI.sendControlChange(3, (uint8_t)round(blob_ptr->centroid.Y), blob_ptr->UID + 1);
